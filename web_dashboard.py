...
def country_lookup(country):
    if country == 'True':
        get_ip_to_country = ip_to_country_code(ssh_creds_log_df)
        top_country = top_10_calculator(get_ip_to_country, "Country_Code")
        message = dbc.Col(dcc.Graph(figure=px.bar(top_country, x="Country_Code", y='frequency')), style={'width': '33%', 'display': 'inline-block'})
    else:
...
app.layout = dbc.Container([
    ...
    dbc.Row([
        dbc.Col(dcc.Graph(figure=px.bar(top_ip_address, y='frequency', title='Top 10 IP Addresses')), width=4),
        dbc.Col(dcc.Graph(figure=px.bar(top_usernames, y='frequency', title='Top 10 Usernames')), width=4),
        dbc.Col(dcc.Graph(figure=px.bar(top_passwords, y='frequency', title='Top 10 Passwords')), width=4),
    ], align='center', class_name='mb-4'),

    # Row 2: Top 10 Commands + Country Codes.
    dbc.Row([
        dbc.Col(dcc.Graph(figure=px.bar(top_cmds, y='frequency', title='Top 10 Commands')), style={'width': '33%', 'display': 'inline-block'}),
