...
# The banner can be changed, it is just a string, a vulnerable version has been selected to lure auditors and/or attackers
    ...
    while True:
        ...
        if char == b'\r':
            if command.strip() == b'exit':
                ...
                response = b"\n" + b'\\usr\\local' + b'\r\n'
                creds_logger.info(f'Command {command.strip()} Client: {client_ip}')
...
class Server(paramiko.ServerInterface):
    ...
    def check_auth_password(self, username, password):
        funnel_logger.info(f'Client {self.client_ip} connection attempt username: {username}, password: {password}')
...
def client_handle(client, addr, username, password):
    ...
    try:
        ...
        transport.local_version = SSH_BANNER  # TODO: Custom banner likely a user-enumerable vulnerable version of SSH
        ...
        standard_banner = "Connection successful! Welcome to the Honeypot!"  # Change this
        channel.send(standard_banner)
        emulated_shell(channel, client_ip=client_ip)

    except AttributeError as error:
        print(error)
        print("Error: Attribute error")  # TODO: better/more verbose error messages
    finally:
        try:
            transport.close()
...
def honeypot(address, port, username, password):
    ...
    sockets.listen(50)  # Number of connections, anything over 100 is unstable
